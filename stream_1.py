...
protected_pages = ["User Login", "Create User", "AdminLogin","AdminDashboard","details"]
...
module1 = sbar.selectbox("📂 Select Module", module_options)
...
def create_user():
    ...
    if create_btn:
        if name.strip() == "" or userid.strip() == "" or password.strip() == "":
            ...
            # ----------------------------
            success = add_user(userid, name,password)
...
def home():
    ...
    with c1:  
        if st.session_state.get("current_username"):
            user_id = st.session_state["current_username"]
            display_name = get_username_from_user_id(user_id)
            print(display_name)
            html = f"""
    <div style="
        line-height:1.1;
    ...
    """
            ...
            with c4:
                log_btn = st.button("👤 Session Logout")
                if log_btn:
...
# 🔹 empty dict
if "movie_json_store" not in st.session_state:
    st.session_state.movie_json_store = {}
movie_json_store = st.session_state.movie_json_store

# ================= MAIN DETAILS PAGE =================
def show_movie_details():
    global movie_json_store
    try:
        movie_id = st.session_state.get("mid")

        if not movie_id:
            st.warning("⚠️ No movie selected.")
            return

        # ✅ movie_json_store se data uthaya
        movie = movie_json_store.get(movie_id, {})

        if not movie:
            st.error("❌ Movie data not available.")
            st.session_state["module"] = "Movie Recommendation"
            return
        ...
        poster_url = (
            ...
            if poster else "images/no_image.png"
        )
        ...
        hero_style = f"""
            <div style="
                background: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('{backdrop_url}') no-repeat center center;
                ...
                margin-bottom: 30px;
            ">
                <h1 style="font-size:42px; margin-bottom:5px;">🎬 {movie.get("title","Unknown Title")}</h1>
                <p style="opacity:0.85;font-size:20px;">{movie.get("tagline","")}</p>
            </div>
        """
        ...
        with col2:
            ...
            st.markdown(f"**Original Title:** {movie.get('original_title','N/A')}")
            st.markdown(f"**Release Date:** {movie.get('release_date','N/A')}")
            st.markdown(f"**Runtime:** {movie.get('runtime','N/A')} min")
            ...
            st.markdown(f"**Adult:** {movie.get('adult', False)}")

            rating = movie.get("vote_average", 0)
            st.markdown(f"**Rating:** {round(rating,1)} / 10")
            st.progress(rating / 10 if rating else 0)
        ...
        if cast:
            ...
            for c, col in zip(cast, cast_cols):
                col.image(
                    f"https://image.tmdb.org/t/p/w185{c.get('profile_path')}"
                    if c.get("profile_path") else "images/no_image.png",
                    use_container_width=True
                )
                col.markdown(f"**{c.get('name')}**\n*{c.get('character')}*")
        else:
        ...
        if directors:
            st.success("🎬 Director: " + ", ".join(directors))

        # ================= TRAILER =================
        st.subheader("🎞 Trailer")
        trailers = movie.get("videos", {}).get("results", [])
        trailer = next((v for v in trailers if v.get("type") == "Trailer"), None)

        if trailer:
            st.video(f"https://www.youtube.com/watch?v={trailer.get('key')}")
        else:
        ...
        companies = ", ".join(
            c.get("name") for c in movie.get("production_companies", [])
        )
        countries = ", ".join(
            c.get("iso_3166_1") for c in movie.get("production_countries", [])
        )
        languages = ", ".join(
            l.get("english_name") for l in movie.get("spoken_languages", [])
        )

        st.markdown(f"**Companies:** {companies or 'N/A'}")
        st.markdown(f"**Countries:** {countries or 'N/A'}")
        st.markdown(f"**Languages:** {languages or 'N/A'}")
            ...
            if r.get("iso_3166_1") == "US":
                for c in r.get("release_dates", []):
                    if c.get("certification"):
                        st.info(f"🇺🇸 US Certification: **{c['certification']}**")
                        found = True
                        break
                break

        if not found:
            st.write("Certification data not available.")

        st.divider()

        # ================= BACK BUTTON =================
        if st.button("⬅ Back to Movie Recommendation"):
            st.session_state["module"] = "Movie Recommendation"
            st.rerun()

    except Exception as e:
        st.error(f"🔥 Unexpected error in details page: {e}")


# -----------------------------
# Load Indian Movies Models/Data
# -----------------------------
ind_movie_model = joblib.load("models/movie_model.pkl")
ind_vector = joblib.load("models/vectors.pkl")
ind_df = joblib.load("models/df.pkl")

# -----------------------------
# Load Hollywood Movies Models/Data
# -----------------------------
hollywood_df = joblib.load("models/hollywood_movie_df_final.pkl")
similarity2 = joblib.load("models/similarity2.pkl")

# -----------------------------
# TMDb Poster Fetch for Hollywood
# -----------------------------

def fetch_hollywood_movie(movie_id):
    """
    Fetch complete Hollywood movie info from TMDb.
    Returns a JSON dict with details, credits, videos.
    ...
    try:
        ...
        params = {
            "api_key": API_KEY,
            "language": "en-US",
            "append_to_response": "videos,images,release_dates,credits"  # include all needed
        }
        resp = requests.get(url_details, params=params, timeout=10)
        resp.raise_for_status()
        movie_data = resp.json()
        
        # -------------------------
        # Extract key simplified info
        # -------------------------
        poster_path = movie_data.get("poster_path")
        movie_data["poster_url"] = (
            f"https://image.tmdb.org/t/p/w500{poster_path}" if poster_path else "images/no_image.png"
        )

        release_date = movie_data.get("release_date", "")
        movie_data["release_year"] = release_date.split("-")[0] if release_date else "?"

        movie_data["plot"] = movie_data.get("overview", "No description available.")

        rating = movie_data.get("vote_average")
        movie_data["rating"] = rating if rating is not None else "?"

        # ✅ Now movie_data contains: 
        # poster_url, release_year, plot, rating + full JSON for credits, videos, etc.
...
# -----------------------------
# Streamlit Recommendation Function
# -----------------------------
def run_movie_rec():
    """
    🎬 Movie Recommendation System (AI Powered) - GRAND UI
    Logic same hai, bas UI upgraded for cinematic experience.
    """
    global movie_json_store

    st.markdown("""
        <h2 style="color:#2f6fed; font-weight:bold;">🎬 Movie Recommendation System</h2>
        <p style="font-size:16px; color:#475569;">
        Discover movies similar to your favorite film instantly and intelligently.<br>
        Recommendations analyze <strong>plot</strong>, <strong>cast</strong>, <strong>director</strong>, and <strong>writer</strong>.
        </p>
    ...
    movie_type = st.radio(
        ...
        ["Indian Movies", "Hollywood Movies"],
        horizontal=True
    )
    ...
    # Load Movie List
    # -----------------------------
    movie_list = ind_df['name'].values if movie_type == "Indian Movies" else hollywood_df['title'].values
    selected_movie = st.selectbox(
        "Select a Movie:",
        ["--- Select Any Movie ---"] + list(movie_list)
    )

    # -----------------------------
    # Recommendation Logic
    # -----------------------------
    if selected_movie == "--- Select Any Movie ---":
        st.warning("⚠️ Please select a movie first.")
        return

    with st.spinner("Finding similar movies... ⏳"):

        if movie_type == "Indian Movies":
            idx = ind_df[ind_df.name == selected_movie].index[0]
            rec_idx = ind_movie_model.kneighbors([ind_vector[idx]], n_neighbors=6)[1][0]
            rec_movies = ind_df.name.iloc[list(rec_idx)[1:]].values
            rec_ids = ind_df.movie_id.iloc[list(rec_idx)[1:]].values
        else:
            idx = hollywood_df[hollywood_df.title == selected_movie].index[0]
            distances = list(enumerate(similarity2[idx]))
            distances = sorted(distances, key=lambda x: x[1], reverse=True)
            rec_movies = hollywood_df.title.iloc[[i[0] for i in distances[1:6]]].values
            rec_ids = hollywood_df.movie_id.iloc[[i[0] for i in distances[1:6]]].values

    st.markdown("## ⭐ Recommended Movies", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)

    # -----------------------------
    # GRAND CSS for cinematic cards
    # -----------------------------
    st.markdown("""
        <style>
        .movie-card {
        ...
        .movie-title {font-size:22px; font-weight:bold; margin-bottom:8px; color:#1f2937;}
        .movie-year {font-size:14px; color:#6b7280; margin-bottom:6px;}
        .movie-plot {font-size:15px; line-height:1.6; color:#374151;}
        .rating-box {
            display:inline-block; background:#FFD700; padding:4px 12px;
    ...
    # -----------------------------
    for m, mid in zip(rec_movies, rec_ids):
        if movie_type == "Indian Movies":
            try:
                resp = requests.get(f"http://www.omdbapi.com/?i={mid}&apikey=48972038").json()
            except:
                resp = {}
            poster = resp.get("Poster", "images/no_image.png")
            plot = resp.get("Plot", "No description available.")
            year = resp.get("Year", "?")
            rating = resp.get("imdbRating", "?")
            col1, col2 = st.columns([1,3])

            with col1:
                st.image(poster if poster != "N/A" else "images/no_image.png", width=160)
                
            with col2:
                st.markdown(f"<div class='movie-card'>", unsafe_allow_html=True)
                st.markdown(f"<div class='movie-title'>{m}</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='movie-year'>📅 {year}</div>", unsafe_allow_html=True)
                st.markdown(f"<div class='rating-box'>⭐ IMDB: {rating}</div>", unsafe_allow_html=True)
                st.markdown(f"<p class='movie-plot'>📘<strong>Plot:</strong> {plot}</p>", unsafe_allow_html=True)
                st.markdown(f"</div>", unsafe_allow_html=True)

            st.markdown("<br>", unsafe_allow_html=True)
        else:
            data = fetch_hollywood_movie(mid)
            if data:
                poster = (
                    "https://image.tmdb.org/t/p/w500/" + data["poster_path"]
                    if data.get("poster_path") else "images/no_image.png"
                )
                plot = data.get("overview", "No description available.")
                year = data["release_date"].split("-")[0] if data.get("release_date") else "?"
                rating = str(data["vote_average"])[:3] if data.get("vote_average") is not None else "?"
                movie_json_store[mid] = data
            else:
                poster, plot, year, rating = "images/no_image.png", "No description available.", "?", "?"
            time.sleep(0.5)
            ...
            with col1:
                st.image(poster if poster != "N/A" else "images/no_image.png", width=160)
                if st.button("🎬 View Details", key=f"movie_btn_{mid}"):
                    st.session_state.mid = mid
                    st.session_state.module = "details"
                    st.rerun()
...
def run_text_correction():
    ...
    st.title("📝 Text Correction Tool (AI Powered)")

    st.markdown("""
    Improve your writing instantly using AI-powered grammar and spelling correction.  
    Supports **single text** and **bulk correction (CSV/TXT)**.
    """)
    ...
    with col1:
        ...
        if st.button("✨ Correct Text"):
            ...
            if text.strip() == "":
                ...
                st.button("📋 Copy Corrected Text")

    # =====================================================
    #                   BULK TEXT CORRECTION
    # =====================================================
    with col2:
        st.subheader("🔹 Bulk Text Correction")

        file = st.file_uploader("Upload CSV/TXT file (1 text per row)", type=["csv", "txt"])

        if file:

            # Read file safely
            try:
                df = pd.read_csv(file, header=None, names=["Original"],sep='#$%')
            except:
                df = pd.read_csv(file, header=None, names=["Original"], sep="\n")

            st.dataframe(df, use_container_width=True)

            if st.button("✨ Correct Bulk Text"):

                placeholder = st.empty()
                placeholder.info("Correcting all rows...")

                df["Corrected"] = df["Original"].apply(lambda x: str(TextBlob(str(x)).correct()))

                placeholder.empty()

                st.success("Bulk correction complete!")
                st.dataframe(df, use_container_width=True)
                ...
                st.download_button(
                    ...
                    data=csv,
                    file_name="corrected_text.csv",
                    mime="text/csv"
                )

                # Summary
                st.markdown("---")
                st.info(f"📊 **Total rows corrected:** {len(df)}")


def detect_language():
    st.markdown("<h3 style='text-align:center;'>🔧 Feature Under Construction</h3>", unsafe_allow_html=True)

    st.image("images/under-construction.png", width=560)
...
# ============ gender_prediction ===================
# ==================================================
def run_Gender_Prediction():


    # ================= SESSION STATE =================
    for key in [
        "portrait", "info_card", "privacy",
        "face_count", "dominant_gender", "anonymize"
    ]:
...
<div style="padding:12px 0;">
  <h2 style="margin-bottom:4px;">🧑‍🦰 Gender Prediction</h2>
  <span style="color:#ff8c00; font-size:14px; font-weight:500;">
    Photo-based · Multi-face Detection
  </span>
</div>
""", unsafe_allow_html=True)
    ...
    # ================= LOAD MODELS =================
    gen_model = joblib.load("models/gender_predict.pkl")
    pcaa = joblib.load("models/gender_pca.pkl")
    ...
    uploaded = st.file_uploader(
        "📁 Upload an image (multiple faces supported)",
        type=["jpg", "jpeg", "png"]
    )
    ...
    if c1.button("🎬 Portrait Mode"):
        st.session_state.portrait = not st.session_state.portrait

    if c2.button("🧾 Info Card"):
        st.session_state.info_card = not st.session_state.info_card

    if c3.button("🔐 Privacy Mode"):
        st.session_state.privacy = not st.session_state.privacy

    if c4.button("👥 Face Count"):
        st.session_state.face_count = not st.session_state.face_count

    if c5.button("🎯 Dominant Gender"):
        st.session_state.dominant_gender = not st.session_state.dominant_gender

    if c6.button("🕶️ Anonymize Faces"):
    ...
    if uploaded:
        ...
        results = []  # store per-face gender

        # ---------- PORTRAIT MODE (CORRECT BLUR) ----------
        if st.session_state.portrait and len(faces) > 0:
            mask = np.zeros_like(fr)
            for (x, y, w, h) in faces:
                mask[y:y+h, x:x+w] = 255
            blurred = cv2.GaussianBlur(fr, (35, 35), 0)
            fr = np.where(mask == 255, fr, blurred)
        ...
        for idx, (x, y, w, h) in enumerate(faces, start=1):
            ...
            face = pcaa.transform([face])

            pred = gen_model.predict(face)[0]
            gender = "Male" if pred.lower() == "male" else "Female"
            results.append(gender)

            # ---------- ANONYMIZATION ----------
            if st.session_state.anonymize:
                roi = fr[y:y+h, x:x+w]
                roi = cv2.resize(roi, (15, 15), interpolation=cv2.INTER_LINEAR)
                roi = cv2.resize(roi, (w, h), interpolation=cv2.INTER_NEAREST)
                fr[y:y+h, x:x+w] = roi

            # ---------- DRAW (IF NOT PRIVACY) ----------
            if not st.session_state.privacy:
                cv2.rectangle(fr, (x, y), (x+w, y+h), (0, 200, 255), 2)
                cv2.putText(
                    fr, f"{gender}",
                    (x, y-10),
        ...
        # ---------- FACE COUNT BADGE ----------
        if st.session_state.face_count:
            cv2.rectangle(fr, (0, 0), (260, 45), (30, 30, 30), -1)
            cv2.putText(
                fr,
        ...
        # ---------- DOMINANT GENDER ----------
        if st.session_state.dominant_gender and results:
            males = results.count("Male")
            females = results.count("Female")
            ...
            cv2.rectangle(fr, (0, 50), (260, 105), (45, 45, 45), -1)
            cv2.putText(
                fr, dom,
                (10, 85),
                cv2.FONT_HERSHEY_SIMPLEX,
        ...
        # ---------- INFO CARD (MULTI FACE) ----------
        if st.session_state.info_card and results:
            h, w, _ = fr.shape
            card_h = 30 + len(results) * 25
            cv2.rectangle(fr, (0, h-card_h), (w, h), (20, 20, 20), -1)

            for i, g in enumerate(results, start=1):
                cv2.putText(
                    fr,
                    f"Face {i}: {g}",
                    (15, h-card_h + 25 + (i-1)*22),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.65, (255, 255, 255), 2
                )

        # ================= DISPLAY =================
        st.image(fr, caption="Processed Output", width=800)
...
elif module=='Movie Recommendation':
    run_movie_rec()
