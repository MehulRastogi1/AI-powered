    ...
    with c1:   
        ...
        if st.session_state["current_username"]:
            ...
            <div style="
                ...
                font-weight:500;
            ">
                Logged in as <b>{st.session_state['current_username']}</b>
            </div>
            """,
            unsafe_allow_html=True
        )
...
# Load models/data
movie_model = joblib.load("models/movie_model.pkl")
vector = joblib.load("models/vectors.pkl")
...
def run_movie_rec():
    ...
    df = joblib.load("models/df.pkl")
    st.markdown("""
    ### 🎬 Movie Recommendation System (AI Powered)

    Discover movies similar to your favorite film — instantly and intelligently.  
    Our recommendations analyze **plot**, **cast**, **director**, and **writer** to find the closest matches.
    """)



    # --------------------------------------------
    # SELECT MOVIE (STYLED HEADING + SUBTEXT)
    # --------------------------------------------
    st.markdown("""
    <div style="
        padding:15px; 
        ...
        margin-bottom:10px;
    ">
        <h3 style="margin:0; color:#1e293b;">🎥 Select a Movie</h3>
        <p style="margin-top:3px; font-size:14px; color:#475569;">
            Choose any movie from the list below to get AI-based recommendations.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🔎 Recommend Movies"):
      if selected_movie == "--- Select ---":
            st.warning("⚠️ Please select any movie first.")
    
      else:
        with st.spinner("Finding similar movies... ⏳"):

            idx = df[df.name == selected_movie].index[0]
            rec = movie_model.kneighbors([vector[idx]], n_neighbors=6)[1][0]

            rec_movies = df.name.iloc[list(rec)[1:]].values
            rec_ids = df.movie_id.iloc[list(rec)[1:]].values
        ...
        for m, mid in zip(rec_movies, rec_ids):
            ...
            with col1:
                if poster and poster != "N/A":
                    ...
                    st.image("images/no_image.png", width=160)
...
def run_Gender_Prediction():
    ...
    if "blur_bg" not in st.session_state:
        st.session_state.blur_bg = False

    # ---------- HEADER ----------
    c1, c2 = st.columns([3, 1])
    with c1:
        st.markdown("## 🧑‍🦰 Live Gender Detection (AI Vision)")
    with c2:
        st.markdown(
            """
            <div style='font-size:13px;color:gray;text-align:right;padding-top:12px'>
            ⚠️ AI-based estimation<br>
            Not 100% accurate
            </div>
            """,
            unsafe_allow_html=True
    ...
    if colb1.button("▶️ Start Camera"):
        st.session_state.camera_on = True

    if colb2.button("⛔ Stop Camera"):
        st.session_state.camera_on = False

    if colb3.button("📸 Take Snapshot"):
        st.session_state.take_snapshot = True

    if colb4.button("🎨 Blur Background"):
        st.session_state.blur_bg = not st.session_state.blur_bg
        st.info(f"Background Blur: {'ON' if st.session_state.blur_bg else 'OFF'}")
    ...
    if st.session_state.camera_on:
        ...
        while vd.isOpened() and st.session_state.camera_on:
            ...
            # ---------- APPLY BLUR ----------
            if st.session_state.blur_bg and len(faces) > 0:
                mask = np.zeros_like(fr)
                for (x, y, w, h) in faces:
                    mask[y:y+h, x:x+w] = 255
                blurred = cv2.GaussianBlur(fr, (21, 21), 0)
            ...
            for i, (x, y, w, h) in enumerate(faces):
                ...
                yhat = gen_model.predict(face)[0]
                emoji = "Male" if yhat.lower() == "male" else "Female"
                label_text = f"{emoji}"
            ...
            # ---------- Snapshot ----------
            if st.session_state.take_snapshot and len(faces) > 0:
                snapshot_dir = "snapshots"
                os.makedirs(snapshot_dir, exist_ok=True)
                filename = f"{snapshot_dir}/snapshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
                cv2.imwrite(filename, fr)
                st.session_state.snapshots.append(filename)
                st.success(f"Snapshot saved: {filename}")
                st.session_state.take_snapshot = False

            # ---------- STREAMLIT DISPLAY ----------
            fr_rgb = cv2.cvtColor(fr, cv2.COLOR_BGR2RGB)
            fr_rgb = cv2.resize(fr_rgb, (500, 500))
            frame_placeholder.image(fr_rgb, channels="RGB")
    ...
    # ---------- STOP MESSAGE ----------
    if st.session_state.camera_on is False:
        st.success("Camera stopped successfully ✔️")
    ...
    if st.session_state.snapshots:
        ...
        try:
            for path in st.session_state.snapshots[-5:][::-1]:  # last 5 snapshots
                    ...
                    with col2:
                        with open(path, "rb") as file:
                            st.download_button(
                                ...
                                mime="image/png",
                                key=path   # unique key zaroori
                            )
                else:
                    st.warning(f"File not found: {path}")

        except Exception:
            st.error("⚠️ Snapshot load failed")



#===================================================
#================== face data prediction ============
#===================================================
def face_prediction():
    ...
    if c1.button("📸 Collect Data"):
        st.session_state.mode = "collect"
        st.session_state.cam_on = False

    if c2.button("🧠 Train Model"):
        st.session_state.mode = "train"
        st.session_state.cam_on = False

    if c3.button("🎥 Live Prediction"):
    ...
    if st.session_state.mode == "collect":
        ...
        st.subheader("📸 Face Data Collection")

        if st.session_state.count == 0:
            st.session_state.name = st.text_input("Enter person name")
        ...
        if start and st.session_state.name != "":
        ...
        if st.session_state.cam_on:
            ...
            while vd.isOpened() and st.session_state.cam_on:
                ...
                if len(faces) > 0 and st.session_state.count < 200:
                    (x,y,w,h) = faces[0]
                    ...
                    st.session_state.X.append(face)
                    st.session_state.y.append(st.session_state.name)
                    st.session_state.count += 1

                    cv2.rectangle(fr,(x,y),(x+w,y+h),(0,0,255),2)

                info_box.info(
                    f"Collected: {st.session_state.count}/200"
                )

                fr = cv2.cvtColor(fr, cv2.COLOR_BGR2RGB)
                frame_box.image(fr, channels="RGB", width=500)

                time.sleep(0.03)

                if st.session_state.count >= 200 or not st.session_state.cam_on:

                    if st.session_state.count >= 200:
                        st.success(
                            f"✅ 200 images collected for {st.session_state.name}"
                        )
                    else:
                        st.warning("⛔ Data collection stopped manually")
    ...
    if st.session_state.mode == "train":
        ...
        if st.button("🚀 Train Now"):
            ...
            st.write("Total samples:", X.shape)
            ...
            # ✅ memory me store
            st.session_state.face_model = model
    ...
    if st.session_state.mode == "predict":
        ...
        if st.session_state.cam_on:
            ...
            model = st.session_state.face_model
