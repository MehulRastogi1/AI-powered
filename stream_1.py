...
def run_spam():
    ...
    # ============= RIGHT COLUMN (BULK CHECK - SPAM) =============
    with col2:
        st.markdown("<div class='section-title'>Bulk Message Check</div>", unsafe_allow_html=True)

        # --- File Upload (unique key) ---
        spam_file = st.file_uploader("Upload your file (.txt or .csv)", type=["txt", "csv"], key="spam_file")

        if spam_file:
            if 'spam_bulk_df' not in st.session_state or st.session_state.get('spam_file_changed', True):
                # Read uploaded file once (adjust separator as needed)
                st.session_state.spam_bulk_df = pd.read_csv(spam_file, header=None, names=["mgs"], sep='$', engine='python')
                st.session_state.spam_predicted = False
                st.session_state.show_spam = False
                st.session_state.show_ham = False
                st.session_state.spam_search_keyword = ''
                st.session_state.spam_file_changed = False

            spam_place = st.empty()
            spam_place.dataframe(st.session_state.spam_bulk_df, use_container_width=True)

        # --- Buttons Row ---
        r_col1, r_col2 = st.columns([1,1])

        with r_col1:
            if st.button("Predict Bulk (Spam)", key="predict_spam") and spam_file:
                # Predict and save in session state
                st.session_state.spam_bulk_df['result'] = model.predict(st.session_state.spam_bulk_df["mgs"])
                st.session_state.spam_predicted = True

        # --- Only show summary/filtering if prediction done ---
        if spam_file and st.session_state.get('spam_predicted', False):

            # --- Checkboxes Row ---
            c1, c2 = st.columns([1,1])
            with c1:
                st.session_state.show_spam = st.checkbox("🛑 Spam", value=st.session_state.show_spam, key="spam_chk_spam")
            with c2:
                st.session_state.show_ham = st.checkbox("✅ Ham", value=st.session_state.show_ham, key="spam_chk_ham")

            # --- Keyword Search with Clear Button ---
            s_col1, s_col2 = st.columns([4,1])
            with s_col1:
                st.session_state.spam_search_keyword = st.text_input(
                    "🔍 Search Keyword", 
                    value=st.session_state.spam_search_keyword,
                    key="spam_search"
                )
            with s_col2:
                st.text('')
                if st.button("❌ Clear", key="spam_clear_search"):
                    st.session_state.spam_search_keyword = ''

            keyword = st.session_state.spam_search_keyword

            # --- Counts ---
            spam_count = (st.session_state.spam_bulk_df['result'] == 'spam').sum()
            ham_count = (st.session_state.spam_bulk_df['result'] == 'ham').sum()
            total = spam_count + ham_count if (spam_count + ham_count) > 0 else len(st.session_state.spam_bulk_df)

            spam_perc = spam_count * 100 / total if total else 0
            ham_perc = ham_count * 100 / total if total else 0
            ...
            filtered_df = st.session_state.spam_bulk_df.copy()
            ...
            spam_place.dataframe(filtered_df.style.apply(highlight_spam_ham, axis=1), use_container_width=True)

            # --- CSV Export ---
            csv_data = filtered_df.to_csv(index=False).encode("utf-8")
            with r_col2:
                st.download_button(
                    label="📥 Download Results CSV (Spam)",
                    data=csv_data,
                    file_name="spam_ham_results.csv",
                    mime="text/csv",
                    key="download_spam_csv"
                )
...
def run_sentiment():
    ...
    with col2:
        ...
        # File upload (unique key)
        sent_file = st.file_uploader("Upload CSV/TXT file (one text per row)", type=["csv", "txt"], key="sent_file")

        if sent_file:
            # Read file once
            if 'sent_bulk_df' not in st.session_state or st.session_state.get('sent_file_changed', True):
                try:
                    st.session_state.sent_bulk_df = pd.read_csv(sent_file, header=None, names=["text"], sep='$#%#', engine='python')
                except:
                    st.session_state.sent_bulk_df = pd.read_csv(sent_file, header=None, names=["text"], sep='\n', engine='python')
                st.session_state.sent_predicted = False
                st.session_state.show_pos = True
                st.session_state.show_neg = True
                st.session_state.sent_search_keyword = ''
                st.session_state.sent_file_changed = False

            sent_place = st.empty()
            sent_place.dataframe(st.session_state.sent_bulk_df, use_container_width=True)

            # --- Predict Bulk Button ---
            r_col1, r_col2 = st.columns([1,1])
            with r_col1:
                if st.button("Predict Bulk (Sentiment)", key="predict_sentiment") and sent_file:
                    placeholder = st.empty()
                    placeholder.info("Analyzing sentiment...")

                    # Predict using your model (make sure `sentiment_model` exists)
                    predictions = sentiment_model.predict(st.session_state.sent_bulk_df["text"])

                    # Convert 0/1 to labels
                    st.session_state.sent_bulk_df['Sentiment'] = ['Positive' if p==1 else 'Negative' for p in predictions]

                    st.session_state.sent_predicted = True
                    placeholder.empty()

            # --- Only show filtering if prediction done ---
            if st.session_state.get('sent_predicted', False):

                # --- Checkboxes for Positive/Negative ---
                c1, c2 = st.columns([1,1])
                with c1:
                    st.session_state.show_pos = st.checkbox("✅ Only Positive", value=st.session_state.show_pos, key="sent_chk_pos")
                with c2:
                    st.session_state.show_neg = st.checkbox("🛑 Only Negative", value=st.session_state.show_neg, key="sent_chk_neg")

                # --- Keyword search ---
                s_col1, s_col2 = st.columns([4,1])
                with s_col1:
                    st.session_state.sent_search_keyword = st.text_input("🔍 Search Keyword", value=st.session_state.sent_search_keyword, key="sent_search")
                with s_col2:
                    st.text('')
                    if st.button("❌ Clear", key="sent_clear_search"):
                        st.session_state.sent_search_keyword = ''

                keyword = st.session_state.sent_search_keyword

                # --- Filter DataFrame based on checkboxes + keyword ---
                filtered_df = st.session_state.sent_bulk_df.copy()
                ...
                sent_place.dataframe(filtered_df.style.apply(highlight_sentiment, axis=1), use_container_width=True)

                # --- CSV Export ---
                csv_data = filtered_df.to_csv(index=False).encode("utf-8")
                with r_col2:
                    st.download_button(
                        label="📥 Download Results CSV (Sentiment)",
                        data=csv_data,
                        file_name="sentiment_results.csv",
                        mime="text/csv",
                        key="download_sent_csv"
                    )
